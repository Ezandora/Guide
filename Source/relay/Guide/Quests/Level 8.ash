...
void QLevel8Init()
{
	...
	if (my_level() >= 8 || my_path_id_legacy() == PATH_EXPLOSIONS)
...
void QLevel8GenerateTasks(ChecklistEntry [int] task_entries, ChecklistEntry [int] optional_task_entries, ChecklistEntry [int] future_task_entries)
{
	...
	else if (!base_quest_state.state_boolean["Past mine"])
	{
		...
		if (need_cheese)
		{
			...
			if ($skill[Advanced Saucecrafting].skill_is_usable() && fullness_limit() > 0 && __misc_state["can eat just about anything"] && my_path_id_legacy() != PATH_SLOW_AND_STEADY)
